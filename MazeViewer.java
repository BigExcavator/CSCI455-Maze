...
public class MazeViewer {
	...
	/**
	 	...
	 	and build the frame of the program
	 * @param args
	 */
   ...
   */
	private static MazeFrame readMazeFile(String fileName) throws IOException {
		...
		//read the relevant information to build the mazeData and start and exit point
		int numRow=readFile.nextInt();
		int numColumn=readFile.nextInt();
		String rowData;
		boolean[][] mazeData=new boolean[numRow][numColumn];
     
		for(int i=0; i<numRow; i++){
			rowData=readFile.next();
			for(int j=0; j<numColumn; j++) {
				if(rowData.charAt(j)==WALL_CHAR) {
					mazeData[i][j]=true;
				}
			}
		}
      
		MazeCoord startLoc=new MazeCoord(readFile.nextInt(), readFile.nextInt());
		MazeCoord exitLoc=new MazeCoord(readFile.nextInt(), readFile.nextInt());
